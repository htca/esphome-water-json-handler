...
class WaterJsonHandler : public Component, public ::AsyncWebHandler {
 public:
  ...
  // Laat initialiseren zodat de web_server zeker klaar is
 ...
 protected:
  ...
  void attach_or_retry_() {
    ...
    if (web_server_base::global_web_server_base != nullptr) {
      ...
      // Webserver nog niet klaar: plan een retry elke 1000 ms
      ...
      this->set_interval(kRetryName, 1000, [this]() {
        if (web_server_base::global_web_server_base != nullptr) {
          ...
          // Stop verdere retries (nu met naam)
...
#else  // Non‑Arduino backend (ESP‑IDF) fallback
